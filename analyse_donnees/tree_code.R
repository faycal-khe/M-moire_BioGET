...
# for E.coli
tre <- read.tree("phylo/ecoli_achtman_4/ecoli2.fasta.contree")
# for Klebsiella
trk <- read.tree("phylo/klebsiella/klebsiella2.fasta.contree")
# for Enterobacter
trc <- read.tree("phylo/ecloacae/enterobacter2.fasta.contree")
tr<-trc
## Reda metadata file

md <- read.xlsx("phylo/reunion_isolates_table.xlsx")
head(md)
...
tr_fig %<+% md +
  geom_tiplab(aes(label=paste0(Host," :: ", ST,study)),size=1.5,align=T)+
  geom_tippoint(aes(colour=Host)) +
  geom_nodepoint(aes(shape=as.numeric(label)>90),size=4)+
  xlim(0,mx*2)+
  scale_colour_brewer(palette="Paired")+
  ggtitle("ESBL E.coli isolated in La Réunion")+
...
a = tr_fig %<+% md +
  geom_tiplab(aes(label=paste0(Host,":: ST", ST," ",study)),size=2.8,align=T)+
  #geom_tiplab(aes(label=paste0("(",y,")")),size=1.3,align=T,offset=0.003,linetype=NA)+
  geom_tippoint(aes(colour=Host)) +
  xlim(0,mx*1.3)+
  scale_colour_brewer(palette="Paired")+
...
# For E.coli
de=read.table("phylo/ecoli_achtman_4/dists.tab", as.is=T, check.names = F)
# For Klebsiella
dk=read.table("phylo/klebsiella/dists.tab", as.is=T, check.names = F)
# For Enterobacter
dc=read.table("phylo/ecloacae/dists.tab", as.is=T, check.names = F)
d1<-dc
...
ds1$dist=cut(ds1$SNPs,c(-1,0,10,50,100,1000,10000,100000,1000000),include.lowest = T)
...
d=ggplot(ds1)+
  geom_tile(aes(x=from,y=to,fill=dist),colour="black")+
   scale_fill_manual(values = c("red","orange","yellow","lightyellow","lightblue","grey70","grey50","grey20","black"),
                    labels=c("0 (identical)","0-100","100-10000","10000-100000",">100000"),name="SNPs")+
  ...
  theme(axis.text.y=element_text(size=6,hjust=1,vjust=0.5),
        axis.text.x.top=element_text(size=6,angle=0,hjust=0.5,vjust=0.5),
