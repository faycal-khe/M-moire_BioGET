...
taux_zone <- tibble(ville = zones_valides) %>%
  left_join(n_total, by = "ville") %>%
  left_join(n_echantillonne, by = "ville") %>%
  mutate(
    n_total = coalesce(n_total, 0L),
    n_echantillonne = coalesce(n_echantillonne, 0L),
    taux = ifelse(n_total > 0, n_echantillonne / n_total, NA_real_),
    valeur_couleur = ifelse(n_echantillonne > 0, taux, NA_real_),
    label = ifelse(
      n_echantillonne > 0,
      paste0(ville, "\n", n_echantillonne, "/", n_total,
...
zones_sf <- communes_zone %>%
  filter(!is.na(ville)) %>%
  group_by(ville, n_echantillonne, n_total, taux, valeur_couleur, label) %>%
...
ggsave("/Users/faycalkhetim/Desktop/STAGE/Annalyse Données/maps/carte_taux_echantillonnage.png",plot = p, width = 12, height = 10, dpi = 320, bg = "white")
plot(p)
